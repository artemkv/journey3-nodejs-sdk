...
import { isSameYear, isSameMonth, isSameDay, isSameHour, } from './dateutil.js';
...
/**
 * Initializes new session.
 * Use this method in you are using this connector with an actual website.
...
export const initializeWeb = async (accountId, appId, version, isRelease) => {
    ...
    try {
        initializing = true;
        const SESSION_TIMEOUT = 30 * 60000; // 30'
        const now = _getNowUtc();
        // if we have current session...
        if (currentSession) {
            // ... but it is stale, re-initialize
            if (now.getTime() > currentSession.end.getTime() + SESSION_TIMEOUT) {
                return initializeInternal(accountId, appId, version, isRelease);
            }
            else {
                // ... else keep the current session
            }
        }
        else {
            // we don't have current session, load the previous one
            const session = await _persistence.loadLastSession(_getNowUtc, _getNewId);
            if (!session) {
                // no stored session -> re-initialize
                return initializeInternal(accountId, appId, version, isRelease);
            }
            // if we have stored session, but it's stale, re-initialize...
            if (now.getTime() > session.end.getTime() + SESSION_TIMEOUT) {
                return initializeInternal(accountId, appId, version, isRelease);
            }
            // ...else use it as current session
...
export const initializeInternal = async (accountId, appId, version, isRelease) => {
    ...
    try {
        // start new session
        const header = createSessionHeader(accountId, appId, version, isRelease, _getNowUtc, _getNewId);
        currentSession = createSession(header.id, accountId, appId, version, isRelease, header.start, _getNowUtc);
...
const reportEventInternal = (eventName, isCollapsible, isError, isCrash) => {
    ...
    if (!currentSession) {
        console.warn('Journey3: Cannot update session. Journey have not been initialized.');
        return;
    }
    try {
        // count events
...
export const reportStageTransition = (stage, stageName) => {
    ...
    if (stage < 1 || stage > 10) {
        throw Error(`Invalid value ${stage} for stage, must be between 1 and 10`);
    }
    try {
